...
def get_mask_properties(
    mask_image, properties=("centroid", "perimeter", "area", "label")
):
...
def single_movie_trajs(image_sequence, max_step=50):
    ...
    for image, frame in zip(image_sequence, range(len(image_sequence))):
        ...
        organoid_data = pd.concat([organoid_data, properties], ignore_index=True)
...
def get_particle_props(dataframe):
    ...
    for particle in dataframe.particle.unique():
        ...
        particleframe["velocity"] = np.sqrt(
            particleframe.dx ** 2 + particleframe.dy ** 2
        )
        ...
        particleframe["absolute_displacement"] = np.sqrt(
            (particleframe.x - xstart) ** 2 + (particleframe.y - ystart) ** 2
        )
